...
{
    const char *p = path;
    const char *dir_s = NULL;
    const char *dir_e = NULL;
    const char *ext_s = NULL;
    ...
    *ext   = 0;
    ...
    while (*p) {
	if (*drive == '\0' && *p == ':') {
	    strncpy(drive, path, p - path + 1);
	    drive[p - path + 1] = '\0';
	    if (*++p == '\\') {
		dir_s = p;
	    }
	    else {
		return; // drive name error
	    }
	}
	else if (*p == '\\') {
	    dir_e = p;
    ...
    if (dir_s != NULL) {
	strncpy(dir, dir_s, dir_e - dir_s + 1);
	dir[dir_e - dir_s + 1] = '\0';
    }
    if (ext_s > dir_e) {
	if (ext_s - dir_e == 1) { 
	    // If the file name is starting from '.', 
	    // we don't consider it as an extension.
	    strncpy(fname, ext_s, p - ext_s);
	    fname[p - ext_s] = '\0';
	    ext_s = p;
	}
	else {
	    strncpy(fname, dir_e + 1, (ext_s - 1) - (dir_e + 1) + 1);
	    fname[(ext_s - 1) - (dir_e + 1) + 1] = '\0';
	}
	strncpy(ext, ext_s, p - ext_s);
	ext[p - ext_s] = '\0';
    }
}
...
static void usage(bool isLong)
{
	...
	"iconic", "ic", "boolean", "start iconic",
